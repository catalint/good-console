...
internals.GoodConsole.prototype._printEvent = function (event) {
    ...
    var data = event.data;
    var output = timestring + ', [' + event.tags.toString() + ']';
    if (event.id !== undefined && this._settings.logId) {
        output += '[' + event.id + ']'
    }
    output += ', ' + data;
...
internals.GoodConsole.prototype._formatResponse = function (event, tags) {
    ...
    var data = [];
    data.push(['%s:', event.instance])
    if (this._settings.logId) {
        data.push(['%s', event.id])
    }
    data.push(['%s', method])
    data.push(['%s', event.path])
    data.push(['%s', query])
    data.push(['%s', statusCode])
    data.push(['(%sms)', event.responseTime])
    data.push(['%s', responsePayload])

    return this._printEvent({
        id: event.id,
        timestamp: event.timestamp,
        tags: tags,
        data: Hoek.format.apply(undefined, [data.map(function(d) {return d[0]}).join(' ')].concat(data.map(function(d) {return d[1]})))
    });
};
